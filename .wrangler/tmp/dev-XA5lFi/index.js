...
// .wrangler/tmp/bundle-Jjs2ui/checked-fetch.js
...
var init_product = __esm({
  "src/services/product.ts"() {
    ...
    ProductService = class {
      ...
      constructor() {
        this.sql = Js(
          "postgresql://inventory_db_owner:bZYGkrhIN94E@ep-muddy-darkness-a17hqwod.ap-southeast-1.aws.neon.tech/inventory_db?sslmode=require"
        );
      }
      async bulkInsert(data) {
        const query = `
    INSERT INTO products (product_id, title, tags, created_at, updated_at, sku) 
    VALUES ($1, $2, $3, $4, $5, $6)
  `;
        try {
      ...
      async deleteProduct(productId) {
        ...
        const query = `DELETE products WHERE id = $1`;
        try {
          if (!product) {
...
var require_products = __commonJS({
  "src/modules/products/index.ts"(exports, module) {
    ...
    var insertVariants = async (c) => {
      const { API_KEY } = env(c);
      const { compatibility_date } = env(c);
      console.log(API_KEY);
      console.log(compatibility_date);
      return c.json({
        success: true
      });
    };
...
// .wrangler/tmp/bundle-Jjs2ui/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-Jjs2ui/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
